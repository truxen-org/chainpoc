...
// Refer to http://wiki.txn.org/index.php/Wire_Protocol
...
func ReadMessage(data []byte) (msg *Msg, remaining []byte, done bool, err error) {
	...
	messageLength := txnutil.BytesToNumber(data[4:8])
	...
	decoder := txnutil.NewValueFromBytes(message)
	// Type of message
...
func WriteMessage(conn net.Conn, msg *Msg) error {
	...
	encoded := txnutil.NewValue(append([]interface{}{byte(msg.Type)}, msg.Data.Slice()...)).Encode()

	if msg.Type == MsgBlockTy {
		txnutil.Log.Debugf("Sending block to %v ... byte[%d] ", conn.RemoteAddr(), len(encoded))
	}

	payloadLength := txnutil.NumberToBytes(uint32(len(encoded)), 32)
	// Write magic token and payload length (first 8 bytes)
	pack = append(MagicToken, payloadLength...)
	pack = append(pack, encoded...)
	// log.Printf("payload %v (%v) %q\n", msg.Type, conn.RemoteAddr(), msg)
