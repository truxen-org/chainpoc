...
func (vm *Vm) Process(contract *Contract, state *State, vars RuntimeVars) {
	...
	Pow256 := txnutil.BigPow(2, 256)

	if txnutil.Config.Debug {
		txnutil.Log.Debugf("#   op\n")
	}
...
out:
	for {
		...
		case oSSTORE:
			y, x := vm.stack.Peekn()
			val := contract.Addr(txnutil.BigToBytes(x, 256))
		...
		if txnutil.Config.Debug {
			txnutil.Log.Debugf("%-3d %-4s", pc, op.String())
		}
		...
		case oLT:
			...
			// x < y
			if x.Cmp(y) < 0 {
				vm.stack.Push(txnutil.BigTrue)
			} else {
				vm.stack.Push(txnutil.BigFalse)
			}
		case oLE:
			x, y := vm.stack.Popn()
			// x <= y
		...
		case oMYADDRESS:
			vm.stack.Push(txnutil.BigD(addr))
		case oTXSENDER:
			vm.stack.Push(txnutil.BigD(vars.sender))
		case oTXVALUE:
		...
		case oTXDATA:
			...
			if v.Cmp(big.NewInt(int64(len(vars.txData)))) >= 0 {
				vm.stack.Push(txnutil.Big("0"))
			} else {
				vm.stack.Push(txnutil.Big(vars.txData[v.Uint64()]))
			}
		case oBLK_PREVHASH:
			vm.stack.Push(txnutil.BigD(vars.prevHash))
		case oBLK_COINBASE:
			vm.stack.Push(txnutil.BigD(vars.coinbase))
		case oBLK_TIMESTAMP:
		...
		case oSHA256, oSHA3, oRIPEMD160:
			...
			for i := 0; i < length; i++ {
				...
				num := txnutil.NumberToBytes(vm.stack.Pop().Bytes(), 256)
				data.WriteString(string(num))
			}

			if op == oSHA256 {
				vm.stack.Push(base.SetBytes(txnutil.Sha256Bin(data.Bytes())))
			} else if op == oSHA3 {
				vm.stack.Push(base.SetBytes(txnutil.Sha3Bin(data.Bytes())))
			} else {
				// vm.stack.Push(base.SetBytes(txnutil.Ripemd160(data.Bytes())))
			}
		case oECMUL:
			y := vm.stack.Pop()
			x := vm.stack.Pop()
			...
			//if txnutil.Big(x).Cmp(txnutil.Big(y)) {
			data := new(bytes.Buffer)
		...
		case oSLOAD:
			...
			decoder := txnutil.NewValueFromBytes([]byte(contract.State().Get(x.String())))
			if !decoder.IsNil() {
		...
		case oSSTORE:
			// Store Y at index X
			y, x := vm.stack.Popn()
			addr := txnutil.BigToBytes(x, 256)
			fmt.Printf(" => %x (%v) @ %v", y.Bytes(), y, txnutil.BigD(addr))
...
func makeInlineTx(addr []byte, value, from, length *big.Int, contract *Contract, state *State) {
	txnutil.Log.Debugf(" => creating inline tx %x %v %v %v", addr, value, from, length)
	j := 0
...
func contractMemory(state *State, contractAddr []byte, memAddr *big.Int) *big.Int {
	...
	// decode the object as a big integer
	decoder := txnutil.NewValueFromBytes([]byte(val))
	if decoder.IsNil() {
		return txnutil.BigFalse
	}
